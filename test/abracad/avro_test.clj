...
(defn roundtrips?
  ...
  ([schema expected input]
   (let [result (and (= expected (apply roundtrip-binary schema input))
                     (= expected (apply roundtrip-json schema input)))]
     (when-not result
       [expected (apply roundtrip-binary schema input) (apply roundtrip-json schema input)])
     result)))
...
(deftest test-example
  (let [schema  {:type      :record,
    ...
    (binding [avro/*avro-readers*, {'abracad.core-test/Example    #'->Example
...
(deftest test-time
  (let [schema                    (avro/parse-schema {:type 'int :logicalType :time-millis})
        ...
        now                       (.truncatedTo (LocalTime/now) ChronoUnit/MILLIS)
...
(deftest test-with-logical-types
  (let [schema  (avro/parse-schema
        ...
        records [{:message-timestamp (.truncatedTo (Instant/now) ChronoUnit/MILLIS)
                  :firstName         "Ronnie",
                  :lastName          "Corbet",
